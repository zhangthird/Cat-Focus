...
export default function ApiSettings({ config, setConfig, remember, setRemember, language }) {
  ...
  const style = { display: 'block', width: '100%', padding: 8, border: '1px solid currentColor', borderRadius: 8, background: 'transparent', marginTop: 4, color: 'inherit' };
  const inputStyle = { ...style, fontFamily: 'ui-monospace, SFMono-Regular, Consolas, "Liberation Mono", monospace', fontSize: 16, lineHeight: 1.5 };
  return <fieldset className="font-hand text-lg" style={{ display: 'grid', gap: 12, minWidth: 0, lineHeight: 1.6 }}>
    <legend className="font-hand text-xl mb-3 border-b-2 border-current inline-block">{zh ? '大模型 API（OpenAI 兼容）' : 'Model API (OpenAI compatible)'}</legend>
    ...
    <label>Base URL<input style={inputStyle} type="url" placeholder="https://your-provider.example/v1" value={config.baseUrl} onChange={e => change('baseUrl', e.target.value)} /></label>
    <label>{zh ? '模型名称' : 'Model'}<input style={inputStyle} value={config.model} placeholder="model-name" onChange={e => change('model', e.target.value)} /></label>
    <label>API Key<input style={inputStyle} type="password" autoComplete="off" value={config.apiKey} placeholder={zh ? '免鉴权服务可留空' : 'Optional for unauthenticated APIs'} onChange={e => change('apiKey', e.target.value)} /></label>
